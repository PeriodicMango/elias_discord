...
import { VISION_ENABLED } from "../../../eliasCore/src/config.js";
import { getMasterId, getMasterTag, setMasterId } from "../../../eliasCore/src/helpers/auth.js";
...
/** Read data.json activePersona as fallback for channels not in channels.json. */
async function readActivePersonaFallback(): Promise<string> {
  try {
...
/** Build a persona-specific view of group chat history. */
function buildFilteredGroupHistory(
  rawMessages: Array<{ role: string; content: string; persona?: string }>,
  ...
  for (const m of rawMessages) {
    ...
    } else if (m.persona === currentPersona) {
      // Own messages: full text
      filtered.push({ role: "assistant", content: m.content });
    } else if (m.persona) {
      // Other persona's message: summarize to prevent style leakage
      filtered.push({
        role: "assistant",
        content: `(摘要) ${summarizePersonaMessage(m.content, m.persona!)}`,
      });
    }
  }
  return filtered;
...
async function handleMessage(message: Message, client: Client): Promise<void> {
  ...
  const persona = routedPersona ?? await readActivePersonaFallback();
