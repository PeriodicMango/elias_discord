...
async function saveWebhookUrl(channelId: string, persona: string, url: string): Promise<void> {
  ...
  try { await fs.mkdir(path.dirname(PATHS.webhooksConfig), { recursive: true }); await fs.writeFile(PATHS.webhooksConfig, JSON.stringify({ entries: webhooks }, null, 2), "utf8"); } catch (err) { console.error("saveWebhookUrl failed:", err); }
}
