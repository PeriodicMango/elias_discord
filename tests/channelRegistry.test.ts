...
describe("personaPath", () => {
  it("returns per-persona path under knowledge_base/Elias/{persona}", () => {
    const result = personaPath("wanshi", "daily_log");
    expect(result).toContain("knowledge_base/Elias/wanshi/daily_log");
    expect(result).toContain("elias"); // parent dir name
  });

  it("joins multiple segments", () => {
    const result = personaPath("elias", "history", "dm.md");
    expect(result).toContain("knowledge_base/Elias/elias/history/dm.md");
  });

  it("works for raw persona", () => {
    const result = personaPath("raw", "status.json");
    expect(result).toContain("knowledge_base/Elias/raw/status.json");
  });
});

describe("sharedPath", () => {
  it("returns shared path under knowledge_base/Elias/shared", () => {
    const result = sharedPath("kb");
    expect(result).toContain("knowledge_base/Elias/shared/kb");
  });

  it("accepts multiple segments", () => {
    const result = sharedPath("persona-memory", "wanshi", "notebook.md");
    expect(result).toContain("knowledge_base/Elias/shared/persona-memory/wanshi/notebook.md");
  });
});
